...
class RfpCliBinaryRunner(ZephyrBinaryRunner):
    ...
    def do_add_parser(cls, parser):
        ...
        parser.add_argument(
            "--rfp-cli", default="rfp-cli", help="The path of rfp-cli program"
        )
    ...
    def do_run(self, command, **kwargs):
        ...
        if self.range:
            for range_ in self.range:
                cmd_flash2 = cmd_flash.copy()
                cmd_flash2.extend(["-range", range_])
                cmd_flash2.extend(["-bin", range_.split(",")[0], self.bin_file])
                self.check_call(cmd_flash2)
        else:
            cmd_flash.extend(["-bin", "0x0", self.bin_file])
