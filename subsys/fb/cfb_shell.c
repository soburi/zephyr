...
static int cmd_display_list(const struct shell *sh, size_t argc, char *argv[])
{
...
static int cmd_display_select(const struct shell *sh, size_t argc, char *argv[])
{
	int idx = strtol(argv[1], NULL, 10);

	if (idx < 0 || ARRAY_SIZE(devices) <= idx) {
		shell_print(sh, "Display: unavailable display id: %d", argv[1]);
	}
