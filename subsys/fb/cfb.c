			    ...
			    struct state_info *state)
{
	switch (param->op) {
	case CFB_OP_FILL:
		fill_fb(fb, state->bg_color, bytes_per_pixel(fb->pixel_format));
		break;
	...
	case CFB_OP_DRAW_LINE:
		draw_line(fb, param->draw_figure.start.x, param->draw_figure.start.y,
			  param->draw_figure.end.x, param->draw_figure.end.y, state->fg_color);
		break;
	case CFB_OP_DRAW_RECT:
		draw_line(fb, param->draw_figure.start.x, param->draw_figure.start.y,
			  param->draw_figure.end.x, param->draw_figure.start.y, state->fg_color);
		...
		draw_line(fb, param->draw_figure.end.x, param->draw_figure.end.y,
			  param->draw_figure.start.x, param->draw_figure.end.y, state->fg_color);
		draw_line(fb, param->draw_figure.start.x, param->draw_figure.end.y,
			  param->draw_figure.start.x, param->draw_figure.start.y, state->fg_color);
		break;
	case CFB_OP_DRAW_TEXT:
		draw_text(fb, param->draw_text.str, param->draw_text.pos.x, param->draw_text.pos.y,
			  false, font_get(state->font_idx), state->kerning, state->fg_color,
			  state->bg_color);
		break;
	case CFB_OP_PRINT:
		draw_text(fb, param->draw_text.str, param->draw_text.pos.x, param->draw_text.pos.y,
			  true, font_get(state->font_idx), state->kerning, state->fg_color,
			  state->bg_color);
		break;
	...
	case CFB_OP_SET_FG_COLOR:
		state->fg_color = rgba_to_color(fb->pixel_format, param->set_color.red,
						param->set_color.green, param->set_color.blue,
						param->set_color.alpha);
		break;
	case CFB_OP_SET_BG_COLOR:
		state->bg_color = rgba_to_color(fb->pixel_format, param->set_color.red,
						param->set_color.green, param->set_color.blue,
						param->set_color.alpha);
		break;
	default:
...
static struct cfb_command_iterator *display_init_iterator(struct cfb_framebuffer *fb)
{
	...
	disp->fb.iterator.disp = disp;
	disp->fb.iterator.node = sys_slist_peek_head(&disp->cmd_list);
	pcmd = CONTAINER_OF(disp->fb.iterator.node, struct cfb_command, node);
	disp->fb.iterator.param = &pcmd->param;
	disp->fb.iterator.next = display_next_iterator;
	disp->fb.iterator.is_last = display_is_last_iterator;

	return &disp->fb.iterator;
}
...
static int display_process_commands(struct cfb_display *disp, uint16_t x, uint16_t y, uint16_t w,
				    uint16_t h, struct state_info *state,
				    enum command_process_mode mode)
{
	...
	const struct state_info save_state = *state;
	int err = 0;
		...
		for (x = start_x; x < draw_width; x += w) {
			*state = save_state;
			for (struct cfb_command_iterator *ite = disp->fb.init_iterator(&disp->fb);
				...
				execute_command(&disp->fb, ite->param, state);
			}
...
static void display_append_init_commands(struct cfb_display *disp)
{
	...
	disp->init_cmds[CFB_INIT_CMD_SET_FONT].param.set_font.font_idx = disp->state.font_idx;

	disp->init_cmds[CFB_INIT_CMD_SET_KERNING].param.op = CFB_OP_SET_KERNING;
	disp->init_cmds[CFB_INIT_CMD_SET_KERNING].param.set_kerning.kerning = disp->state.kerning;

	color_to_rgba(disp->fb.pixel_format, disp->state.fg_color, &r, &g, &b, &a);
	...
	color_to_rgba(disp->fb.pixel_format, disp->state.bg_color, &r, &g, &b, &a);
			    ...
			    uint16_t height)
{
	...
	return display_process_commands(disp, MAX(x, 0), MAX(y, 0), MIN(width, fb->res.x - x),
					MIN(height, fb->res.y - y), &disp->state, FINALIZE);
}
...
static int display_clear(struct cfb_framebuffer *fb, bool clear_display)
{
	...
	if (disp->fb.size < fb_screen_buf_size(&disp->fb)) {
		...
		err = display_process_commands(disp, 0, 0, fb->res.x, fb->res.y, &disp->state,
					       CLEAR_COMMANDS);
	} else {
...
/**
 * Immediately render to buffer
 *
 * @param fb framebuffer pointer that is linked to display
 *
 * @return negative value if failed, otherwise 0
 */
static int display_immediate(struct cfb_display *disp)
{
	int err = display_process_commands(disp, 0, 0, disp->fb.res.x, disp->fb.res.y, &disp->state,
					   IMMEDIATE);
...
int cfb_display_init(struct cfb_display *disp, const struct cfb_display_init_param *param)
{
	...
	disp->dev = param->dev;
	disp->state.font_idx = 0U;
	disp->state.kerning = 0U;

	if (cfg.current_pixel_format == PIXEL_FORMAT_MONO10) {
		disp->state.bg_color = 0xFFFFFFFFU;
		disp->state.fg_color = 0x0U;
	} else {
		disp->state.fg_color = 0xFFFFFFFFU;
		disp->state.bg_color = 0x0U;
	}
	...
	fill_fb(&disp->fb, disp->state.bg_color, bytes_per_pixel(disp->fb.pixel_format));
