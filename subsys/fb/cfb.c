...
static inline uint32_t fb_size(const struct cfb_framebuffer *fb)
{
	return fb->width * fb->height * fb_bpp(fb) / fb_ppt(fb);
}
...
static inline bool iterator_is_last(struct command_iterator ite)
{
	return !ite.node &&
	       (!ite.disp->cmd.buf ||
		((void *)ite.param >= (void *)(ite.disp->cmd.buf + ite.disp->cmd.size)));
}
...
static inline struct cfb_command_param *iterator_get_param(struct command_iterator ite)
{
	...
	return !ite.node ? ite.param : &pcmd->param;
}
...
static struct command_iterator next_iterator(struct command_iterator ite)
{
	...
	if (ite.node != NULL) {
		next.disp = ite.disp;
		next.node = ite.node->next;
		next.param = NULL;
		if (next.node == NULL) {
			next.param = (void *)ite.disp->cmd.buf;
		}
	} else {
			    ...
			    uint16_t height)
{
	...
	ite.param = (struct cfb_command_param *)disp->cmd.buf;
...
static int display_clear(const struct cfb_framebuffer *fb, bool clear_display)
{
	...
	if (disp->fb.size < fb_size(&disp->fb)) {
...
static int display_immediate(const struct cfb_framebuffer *fb)
{
	struct cfb_display *disp = CONTAINER_OF(fb, struct cfb_display, fb);
	struct state_info state = {
		.font_idx = &disp->font_idx,
	...
	err = process_command_list(&info, 0, 0, disp->x_res, disp->y_res, &state, ite, IMMEDIATE);
				  ...
				  bool append_buffer)
{
	...
	if (!disp->cmd.buf) {
		...
		memcpy(&disp->cmd.buf[disp->cmd.pos], &cmd->param,
		       sizeof(struct cfb_command_param));
...
int cfb_get_display_parameter(const struct cfb_display *disp, enum cfb_display_param param)
{
	...
	case CFB_DISPLAY_ROWS:
		if (disp->fb.screen_info & SCREEN_INFO_MONO_VTILED) {
			return disp->y_res / pixels_per_tile(disp->fb.pixel_format);
		}
		return disp->y_res;
	case CFB_DISPLAY_COLS:
		...
		return disp->x_res / pixels_per_tile(disp->fb.pixel_format);
	}
	return 0;
}
...
int cfb_display_init(struct cfb_display *disp, const struct cfb_display_init_param *param)
{
	...
	if (param->transfer_buf_size < fb_size(&disp->fb)) {
		if (!disp->cmd.buf) {
			return -EINVAL;
		}
	} else {
	...
	info.screen.x = disp->x_res;
	info.screen.y = disp->y_res;
	info.fb = &disp->fb;
	fill_fb(&info, disp->bg_color, bytes_per_pixel(disp->fb.pixel_format));
