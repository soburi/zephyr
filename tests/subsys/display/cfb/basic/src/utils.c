...
#if CONFIG_TEST_TRANSFER_BUF_SIZE != 0
	zassert_ok(cfb_init(&fb, dev, &param));
	return &fb;
#else
	pfb = cfb_alloc(dev);
	zassert_not_null(pfb);
	return pfb;
#endif
}

void fb_deinit(struct cfb_framebuffer *fb)
{
#if CONFIG_TEST_TRANSFER_BUF_SIZE != 0
	cfb_deinit(fb);
#else
