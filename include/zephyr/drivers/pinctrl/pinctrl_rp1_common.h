...
#define RP1_GET_PIN_NUM(pinctrl)  (((pinctrl) >> RP1_PINMUX_PIN_POS) & RP1_PINMUX_PIN_MASK)
#define RP1_GET_ALT_FUNC(pinctrl) (((pinctrl) >> RP1_PINMUX_FUNC_POS) & RP1_PINMUX_FUNC_MASK)

#define RP1_PINCTRL_STATE_PIN_INIT(node_id, prop, idx)                                             \
	{                                                                                          \
		.pin_num = RP1_GET_PIN_NUM(DT_PROP_BY_IDX(node_id, prop, idx)),                    \
		.alt_func = RP1_GET_ALT_FUNC(DT_PROP_BY_IDX(node_id, prop, idx)),                  \
		.f_m = DT_PROP(node_id, raspberrypi_oe_override),                                  \
