...
typedef uint64_t gpio_port_pins_64bit_t;
typedef uint32_t gpio_port_pins_32bit_t;
 ...
 */
#ifdef CONFIG_GPIO_64BIT_PORT
typedef gpio_port_pins_64bit_t gpio_port_pins_t;
#else
typedef gpio_port_pins_32bit_t gpio_port_pins_t;
#endif

typedef uint64_t gpio_port_value_64bit_t;
typedef uint32_t gpio_port_value_32bit_t;
 ...
 */
#ifdef CONFIG_GPIO_64BIT_PORT
typedef gpio_port_value_64bit_t gpio_port_value_t;
#else
typedef gpio_port_value_32bit_t gpio_port_value_t;
#endif
						      ...
						      gpio_flags_t flags)
{
	...
	__ASSERT((cfg->port_pin_mask & (gpio_port_pins_t)GPIO_BIT(pin)) != 0U,
		 "Unsupported pin");

	if (((flags & GPIO_INT_LEVELS_LOGICAL) != 0) &&
	    ((data->invert & (gpio_port_pins_t)GPIO_BIT(pin)) != 0)) {
		/* Invert signal bits */
		flags ^= (GPIO_INT_LOW_0 | GPIO_INT_HIGH_1);
	}
					    ...
					    gpio_flags_t flags)
{
	...
	if ((flags & GPIO_ACTIVE_LOW) != 0) {
		data->invert |= (gpio_port_pins_t)GPIO_BIT(pin);
	} else {
		data->invert &= ~(gpio_port_pins_t)GPIO_BIT(pin);
	}
...
static inline int gpio_pin_is_input(const struct device *port, gpio_pin_t pin)
{
	...
	__ASSERT((cfg->port_pin_mask & (gpio_port_pins_t)GPIO_BIT(pin)) != 0U, "Unsupported pin");
	...
	return (int)!!((gpio_port_pins_t)GPIO_BIT(pin) & pins);
}
...
static inline int gpio_pin_get_raw(const struct device *port, gpio_pin_t pin)
{
	...
	ret = gpio_port_get_raw(port, &value);
	if (ret == 0) {
		ret = (value & (gpio_port_pins_t)GPIO_BIT(pin)) != 0 ? 1 : 0;
	}
				   ...
				   int value)
{
	...
	if (value != 0)	{
		ret = gpio_port_set_bits_raw(port, (gpio_port_pins_t)GPIO_BIT(pin));
	} else {
		ret = gpio_port_clear_bits_raw(port, (gpio_port_pins_t)GPIO_BIT(pin));
	}
			       ...
			       int value)
{
	...
	if (data->invert & (gpio_port_pins_t)GPIO_BIT(pin)) {
		value = (value != 0) ? 0 : 1;
	}
...
static inline int gpio_pin_toggle(const struct device *port, gpio_pin_t pin)
{
	...
	return gpio_port_toggle_bits(port, (gpio_port_pins_t)GPIO_BIT(pin));
}
