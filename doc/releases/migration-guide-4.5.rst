...
* The standalone Raspberry Pi RP1 GPIO driver (``CONFIG_GPIO_RP1`` / ``gpio_rp1.c``) has been
  ...
  GPIO driver (``CONFIG_GPIO_RPI_PICO``), which is selected automatically when such a node is
  enabled. Out-of-tree configurations that set ``CONFIG_GPIO_RP1`` should drop it; the symbol no
  longer exists.
