...
static void uart_ra_sci_irq_err_enable(const struct device *dev)
{
	...
	ra_icu_event_enable(cfg->eri_event);
}
...
static void uart_ra_sci_irq_err_disable(const struct device *dev)
{
	...
	ra_icu_event_disable(cfg->eri_event);
}
...
static void uart_ra_sci_rxi_isr(const struct device *dev)
{
	...
	ra_icu_event_clear_flag(config->rxi_event);
}
...
static void uart_ra_sci_txi_isr(const struct device *dev)
{
	...
	ra_icu_event_clear_flag(config->txi_event);
}
...
static void uart_ra_sci_tei_isr(const struct device *dev)
{
	...
	ra_icu_event_clear_flag(config->tei_event);
}
...
static void uart_ra_sci_eri_isr(const struct device *dev)
{
	...
	ra_icu_event_clear_flag(config->eri_event);
}
...
#define UART_RA_SCI_IRQ_CONFIG_INIT(index)                                                         \
	do {                                                                                       \
		...
		ra_icu_event_enable(DT_IRQ_BY_NAME(DT_INST_PARENT(index), rxi, event));            \
		...
		ra_icu_event_enable(DT_IRQ_BY_NAME(DT_INST_PARENT(index), txi, event));            \
		...
		ra_icu_event_enable(DT_IRQ_BY_NAME(DT_INST_PARENT(index), tei, event));            \
		...
		ra_icu_event_enable(DT_IRQ_BY_NAME(DT_INST_PARENT(index), eri, event));            \
	} while (0)
...
#define UART_RA_SCI_INIT(index)                                                                    \
	...
	{                                                                                          \
		...
		data->fsp_config.rxi_irq = __intr_id_table[DT_IRQ_BY_NAME(DT_INST_PARENT(index), rxi, event)];      \
		data->fsp_config.txi_ipl = DT_IRQ_BY_NAME(DT_INST_PARENT(index), txi, priority);   \
		data->fsp_config.txi_irq = __intr_id_table[DT_IRQ_BY_NAME(DT_INST_PARENT(index), txi, event)];      \
		data->fsp_config.tei_ipl = DT_IRQ_BY_NAME(DT_INST_PARENT(index), tei, priority);   \
		data->fsp_config.tei_irq = __intr_id_table[DT_IRQ_BY_NAME(DT_INST_PARENT(index), tei, event)];      \
		data->fsp_config.eri_ipl = DT_IRQ_BY_NAME(DT_INST_PARENT(index), eri, priority);   \
		data->fsp_config.eri_irq = __intr_id_table[DT_IRQ_BY_NAME(DT_INST_PARENT(index), eri, event)];      \
