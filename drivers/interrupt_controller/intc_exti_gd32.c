...
REGS_FSCOPE
FlagStatus exti_interrupt_flag_get(gd32_exti_line_enum linex)
{
...
REGS_FSCOPE
void exti_interrupt_disable(gd32_exti_line_enum linex)
{
    EXTI_INTEN &= ~(uint32_t) linex;
}

REGS_FSCOPE
void exti_interrupt_flag_clear(gd32_exti_line_enum linex)
{
    EXTI_PD = (uint32_t) linex;
}

REGS_FSCOPE
void exti_interrupt_enable(gd32_exti_line_enum linex)
{
	/* set the EXTI mode and enable the interrupts or events from EXTI line x */
	EXTI_INTEN |= (uint32_t) linex;
}

static void exti_set_rising_trigger(gd32_exti_line_enum linex)
{
	EXTI_RTEN |= (uint32_t) linex;
}

static void exti_reset_rising_trigger(gd32_exti_line_enum linex)
{
	EXTI_RTEN &= ~(uint32_t) linex;
}

static void exti_set_falling_trigger(gd32_exti_line_enum linex)
{
	EXTI_FTEN |= (uint32_t) linex;
}

static void exti_reset_falling_trigger(gd32_exti_line_enum linex)
{
	EXTI_FTEN &= ~(uint32_t) linex;
}
