...
#define WS2812_RPI_PICO_PIO_INIT(inst)                                                             \
	static const uint8_t ws2812_rpi_pico_pio_##inst##_color_mapping[] =                        \
		DT_INST_PROP(inst, color_mapping);                                                 \
	struct ws2812_rpi_pico_pio_data ws2812_rpi_pico_pio_##inst##_data;                         \
                                                                                                   \
	static const struct ws2812_rpi_pico_pio_config ws2812_rpi_pico_pio_##inst##_config = {     \
		.piodev = DEVICE_DT_GET(DT_PARENT(DT_INST_PARENT(inst))),                          \
		.output_pin = DT_INST_PROP(inst, output_pin),                                      \
		.num_colors = DT_INST_PROP_LEN(inst, color_mapping),                               \
		.color_mapping = ws2812_rpi_pico_pio_##inst##_color_mapping,                       \
		.reset_delay = DT_INST_PROP(inst, reset_delay),                                    \
		.frequency = DT_INST_PROP(inst, frequency),                                        \
		.cycles_per_bit = CYCLES_PER_BIT(DT_INST_PARENT(inst)),                            \
	};                                                                                         \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, &ws2812_rpi_pico_pio_init, NULL,                               \
			      &ws2812_rpi_pico_pio_##inst##_data,                                  \
			      &ws2812_rpi_pico_pio_##inst##_config, POST_KERNEL,                   \
			      CONFIG_LED_STRIP_INIT_PRIORITY, &ws2812_rpi_pico_pio_api);
