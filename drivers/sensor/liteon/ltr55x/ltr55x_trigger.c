...
static void ltr55x_gpio_callback(const struct device *dev, struct gpio_callback *cb,
				 uint32_t pin_mask)
{
		       ...
		       sensor_trigger_handler_t handler)
{
	...
	rc = i2c_reg_update_byte_dt(&config->bus, LTR55X_INTERRUPT,
				    LTR55X_INTERRUPT_PS_MASK | LTR55X_INTERRUPT_ALS_MASK,
				    interrupt);
	if (rc < 0) {
		return rc;
	}
...
int ltr55x_trigger_init(const struct device *dev)
{
	...
	k_thread_create(&data->thread, data->thread_stack, CONFIG_LTR55X_THREAD_STACK_SIZE,
			ltr55x_thread_main, data, NULL, NULL,
			K_PRIO_COOP(CONFIG_LTR55X_THREAD_PRIORITY), 0, K_NO_WAIT);
	...
	gpio_init_callback(&data->gpio_cb, ltr55x_gpio_callback, BIT(config->int_gpio.pin));
