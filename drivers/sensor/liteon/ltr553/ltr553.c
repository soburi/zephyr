...
static int ltr553_channel_get(const struct device *dev, enum sensor_channel chan,
...
static int ltr553_init(const struct device *dev)
{
	const struct ltr553_config *cfg = dev->config;
	uint8_t value;
	int ret;
	...
	ret = i2c_reg_read_byte_dt(&cfg->bus, LTR553_REG_PART_ID, &value);
	if (ret < 0) {
	...
	ret = i2c_reg_write_byte_dt(
		&config->bus, LTR553_REG_ALS_CONTROL,
		LTR553_ALS_CONTROL_MODE_ACTIVE |
			(LTR553_ALS_GAIN_CODE << LTR553_ALS_CONTROL_GAIN_SHIFT));
	if (ret < 0) {
	...
	ret = i2c_reg_write_byte_dt(&config->bus, LTR553_REG_PS_LED,
				    LTR553_PS_LED_FREQ_CODE | LTR553_PS_LED_DUTY_CODE |
					    LTR553_PS_LED_CURRENT_CODE);
	if (ret < 0) {
	...
	ret = i2c_reg_write_byte_dt(&config->bus, LTR553_REG_PS_MEAS_RATE,
				    LTR553_PS_MEAS_RATE_CODE);
	if (ret < 0) {
...
#define DEFINE_LTR553(inst)                                                                        \
	...
	static const struct ltr553_config ltr553_config_##inst = {                                 \
		.bus = I2C_DT_SPEC_INST_GET(inst),                                                 \
                                                                                                   ...
                                                                                                   \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, ltr553_init, NULL, &ltr553_data_##inst,            \
				     &ltr553_config_##inst, POST_KERNEL,                           \
				     CONFIG_SENSOR_INIT_PRIORITY, &ltr553_driver_api);

DT_INST_FOREACH_STATUS_OKAY(DEFINE_LTR553)
