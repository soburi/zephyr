...
#include <zephyr/drivers/syscon/qingke.h>
					      ...
					      enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	...
	syscon_read_reg(QINGKE_SYSCON, SYSCON_GPIO_INT_ENABLE, &regval);
	regval &= ~BIT(offset);
	syscon_write_reg(QINGKE_SYSCON, SYSCON_GPIO_INT_ENABLE, regval);
	...
	/* Set interrupt mode */
	syscon_read_reg(QINGKE_SYSCON, SYSCON_GPIO_INT_MODE, &regval);
	if (mode == GPIO_INT_MODE_EDGE) {
		regval |= BIT(offset);
	} else {
		regval &= ~BIT(offset);
	}
	syscon_write_reg(QINGKE_SYSCON, SYSCON_GPIO_INT_MODE, regval);

	/* Set interrupt trigger */
	syscon_read_reg(QINGKE_SYSCON, SYSCON_GPIO_INT_POLAR, &regval);
	if (trig == GPIO_INT_TRIG_HIGH) {
		regval |= BIT(offset);
	} else if (trig == GPIO_INT_TRIG_LOW) {
		regval &= ~BIT(offset);
	}
	syscon_write_reg(QINGKE_SYSCON, SYSCON_GPIO_INT_POLAR, regval);
...
static void gpio_ch56x_isr(const struct device *port)
{
	...
	syscon_read_reg(QINGKE_SYSCON, SYSCON_GPIO_INT_FLAG, &regval);
	for (uint32_t i = 0; i < cfg->int_pins_cnt; i++) {
	...
	/* Clear interrupt */
	syscon_write_reg(QINGKE_SYSCON, SYSCON_GPIO_INT_FLAG, regset);
