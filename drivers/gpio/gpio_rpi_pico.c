...
#define GPIO_RPI_MMIO_INIT(node_id, prop, idx)                                                     \
...
#define GPIO_RPI_COMMON_INIT(node_id)                                                              \
	...
	IF_ENABLED(DT_NODE_HAS_PROP(DT_PARENT(node_id), reg_names), (                              \
		...
		{                                                                                  \
			...
			DT_FOREACH_PROP_ELEM(DT_PARENT(node_id), reg_names, GPIO_RPI_MMIO_MAP)     \
		}                                                                                  \
	))                                                                                         \
	static const struct gpio_rpi_config gpio_rpi_##node_id##_config = {                        \
		...
		IF_ENABLED(DT_NODE_HAS_PROP(DT_PARENT(node_id), reg_names), (                      \
			DT_FOREACH_PROP_ELEM(DT_PARENT(node_id), reg_names, GPIO_RPI_MMIO_INIT)    \
...
#define GPIO_RP1_INIT(node_id)                                                                     \
	IF_DISABLED(IS_GPIO_RPI_LO_NODE(node_id), (                                                \
		BUILD_ASSERT(0, "raspberrypi,rp1-gpio only allows the use of bank0.");             \
	))                                                                                         \
