...
#define SYSCON_CLK_PLL_DIV(base)  (base + 0x00)
#define SYSCON_CLK_CFG_CTRL(base) (base + 0x02)

/* SYSCON_CLK_PLL_DIV Register */
#define SYSCON_CLK_PLL_DIV_KEY (0x40)

/* SYSCON_CLK_CFG_CTRL Register */
#define SYSCON_CLK_CFG_CTRL_KEY (0x80)
...
static int ch56x_clkmux_init(const struct device *dev)
{
	...
	syscon_write_reg_unlock(cfg->syscon, SYSCON_CLK_PLL_DIV(cfg->base),
				SYSCON_CLK_PLL_DIV_KEY | divider);
	syscon_write_reg_unlock(cfg->syscon, SYSCON_CLK_CFG_CTRL(cfg->base),
				SYSCON_CLK_CFG_CTRL_KEY | source);
