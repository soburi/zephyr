...
{
	rcu_periph_clock_enable((rcu_periph_enum)RCU_REGIDX_BIT(en_offset[bus], RCU_BIT_POS(periph)));
}

static inline void periph_clock_disable(uint32_t bus, uint32_t periph)
{
	rcu_periph_clock_disable((rcu_periph_enum)RCU_REGIDX_BIT(en_offset[bus], RCU_BIT_POS(periph)));
}
