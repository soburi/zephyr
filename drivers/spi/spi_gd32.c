...
static inline uint32_t spi_i2s_status_regs(uint32_t spi_periph)
{
...
{
	uint32_t sr = spi_i2s_status_regs(BASE_ADDR(spi));
...
static struct spi_gd32_data spi_gd32_dev_data_##id = {		\
	...
	SPI_DMA_STATUS_SEM(id)						\
									...
									\
DEVICE_DT_INST_DEFINE(id, &spi_gd32_init, device_pm_control_nop,	\
