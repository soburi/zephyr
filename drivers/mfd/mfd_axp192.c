...
#define AXP_BOTH_ENABLED11 1

#define AXP_MODEL_COND(cond_both_192_2101, cond_192, cond_2101) \
	COND_CODE_1(UTIL_CAT(UTIL_CAT(AXP_BOTH_ENABLED,CONFIG_DT_HAS_X_POWERS_AXP192_ENABLED), CONFIG_DT_HAS_X_POWERS_AXP2101_ENABLED), \
		   (cond_both_192_2101), \
		   (COND_CODE_1(CONFIG_DT_HAS_X_POWERS_AXP192_ENABLED, (cond_192), (cond_2101))))

#define REG_CHIP_ID(config) AXP_MODEL_COND(config->reg_chip_id, AXP192_REG_CHIP_ID, AXP2101_REG_CHIP_ID)
#define CHIP_ID(config) AXP_MODEL_COND(config->reg_chip_id, AXP192_CHIP_ID, AXP2101_CHIP_ID)
#define VBUS_CFG_REG(config) AXP_MODEL_COND(config->reg_vbus_config, AXP192_VBUS_CFG_REG, AXP2101_VBUS_CFG_REG)
#define VBUS_CFG_VAL_VBUSEN_DISABLE(config) AXP_MODEL_COND(config->reg_chip_id, AXP192_VBUS_CFG_VAL_VBUSEN_DISABLE, AXP2101_VBUS_CFG_VAL_VBUSEN_DISABLE)
...
static int mfd_axp192_init(const struct device *dev)
{
	...
	ret = i2c_reg_read_byte_dt(&config->i2c, REG_CHIP_ID(config), &chip_id);
	if (ret < 0) {
		return ret;
	}
	if (chip_id != CHIP_ID(config)) {
		LOG_ERR("Invalid Chip detected (%d)", chip_id);
		return -EINVAL;
	}
#ifdef CONFIG_DT_HAS_X_POWERS_AXP192_ENABLED
	/* Disable N_VBUSEN */
	ret = i2c_reg_update_byte_dt(&config->i2c, VBUS_CFG_REG(config),
				     VBUS_CFG_VAL_VBUSEN_DISABLE(config),
				     config->vbusen_disable ? VBUS_CFG_VAL_VBUSEN_DISABLE(config) : 0);
	if (ret < 0) {
		return ret;
	}
#endif
...
#define MFD_AXP192_DEFINE(node)                                                                    \
	static const struct mfd_axp192_config config##node = {                                     \
		...
		IF_ENABLED(DT_ANY_INST_HAS_PROP_STATUS_OKAY(x_powers_axp192), \
	...
	COND_CODE_1(DT_ANY_INST_HAS_PROP_STATUS_OKAY(x_powers_axp192), (&data##node), (NULL)), \
			 &config##node, POST_KERNEL, CONFIG_MFD_INIT_PRIORITY, NULL);

DT_FOREACH_STATUS_OKAY(x_powers_axp192, MFD_AXP192_DEFINE);
DT_FOREACH_STATUS_OKAY(x_powers_axp2101, MFD_AXP192_DEFINE);
