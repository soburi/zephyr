...
#define AXP2101_CHGLED_BLINK_SLOW                                                                  \
	(AXP2101_CHGLED_OUTPUT_BLINK_SLOW << AXP2101_CHGLED_OUTPUT_OFFSET) |                       \
		(AXP2101_CHGLED_CTRL_BY_REG << AXP2101_CHGLED_CTRL_OFFSET) |                       \
		(AXP2101_CHGLED_PIN_ENABLE << AXP2101_CHGLED_PIN_OFFSET)

#define AXP2101_CHGLED_BLINK_FAST                                                                  \
	(AXP2101_CHGLED_OUTPUT_BLINK_FAST << AXP2101_CHGLED_OUTPUT_OFFSET) |                       \
		(AXP2101_CHGLED_CTRL_BY_REG << AXP2101_CHGLED_CTRL_OFFSET) |                       \
		(AXP2101_CHGLED_PIN_ENABLE << AXP2101_CHGLED_PIN_OFFSET)

#define AXP2101_CHGLED_TYPE_A                                                                      \
	(AXP2101_CHGLED_CTRL_BY_REG << AXP2101_CHGLED_CTRL_TYPE_A) |                               \
		(AXP2101_CHGLED_PIN_ENABLE << AXP2101_CHGLED_PIN_OFFSET)

#define AXP2101_CHGLED_TYPE_B                                                                      \
	(AXP2101_CHGLED_CTRL_BY_REG << AXP2101_CHGLED_CTRL_TYPE_B) |                               \
		(AXP2101_CHGLED_PIN_ENABLE << AXP2101_CHGLED_PIN_OFFSET)

#define DELAY_ON_SLOW_BLINK  (((1000 / 4) / 1) * 1)
#define DELAY_OFF_SLOW_BLINK (((1000 / 4) / 1) * 3)
#define DELAY_ON_FAST_BLINK  (((1000 / 4) / 4) * 1)
#define DELAY_OFF_FAST_BLINK (((1000 / 4) / 4) * 3)
			     ...
			     uint32_t delay_off)
{
	...
	if ((delay_on == DELAY_ON_SLOW_BLINK) && (delay_off == DELAY_OFF_SLOW_BLINK)) {
		return i2c_reg_write_byte_dt(&config->i2c, AXP2101_REG_CHGLED,
					     AXP2101_CHGLED_OUTPUT_SLOW_BLINK);
	} else if ((delay_on == DELAY_ON_FAST_BLINK) && (delay_off == DELAY_OFF_FAST_BLINK)) {
		return i2c_reg_write_byte_dt(&config->i2c, AXP2101_REG_CHGLED,
					     AXP2101_CHGLED_OUTPUT_FAST_BLINK);
	}
