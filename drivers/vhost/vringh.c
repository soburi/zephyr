		   ...
		   uint16_t *head_out)
{
	...
	struct vhost_buf desc_ranges[CONFIG_VHOST_CHAIN_BUFS_MAX];
	...
	do {
		...
		if (count >= ARRAY_SIZE(desc_ranges)) {
			LOG_ERR("Descriptor chain longer than VHOST_CHAIN_BUFS_MAX: %zu", count);
			ret = -E2BIG;
			goto failed;
		}
