...
static struct vhost_iovec riovec[RNG_QUEUE_SIZE];
static struct vhost_iovec wiovec[RNG_QUEUE_SIZE];
static struct vhost_buf desc_bufs[RNG_QUEUE_SIZE];

static struct vringh_iov riov = {
	.iov = riovec,
	.max_num = RNG_QUEUE_SIZE,
};
...
{
	LOG_DBG("%s: queue_id=%u", __func__, vrh->queue_id);
	uint16_t head;
...
void rng_queue_ready(const struct device *dev, uint16_t qid, void *data)
{
	...
	int err = vringh_init_device(&vrh_inst, dev, qid, desc_bufs, RNG_QUEUE_SIZE,
				     vringh_kick_handler);
